...
This skill generates a quiz based on a git diff to help verify that a human
...
### Format 2: JSON file (for use with quiztool)

Generate a JSON file that can be run with `scripts/quiztool`:

```json
{
  "title": "Diff Quiz: [Brief description]",
  "difficulty": "medium",
  "commit": "abc123",
  "estimated_minutes": 15,
  "questions": [
    {
      "text": "Explain in your own words what this function does.",
      "expected_type": "Explanation"
    },
    {
      "text": "What error conditions does this code handle?",
      "expected_type": "Short answer"
    }
  ]
}
```

Save to a file like `quiz-<commit-short>.json`, then run:

```bash
scripts/quiztool run quiz-abc123.json
```

The tool prompts for answers interactively and saves responses to a JSON file
for later grading.

### Format 3: Bash Script (`--script` or "as a script")
